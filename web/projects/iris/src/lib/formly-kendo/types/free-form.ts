...
import { isNotNilOrEmpty, reviver } from '../../core/utility';
...
export class FormlyFieldFreeFormComponent extends FieldType implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (isNotNilOrEmpty(this.to['parentFieldName'])) {
      if (isNotNilOrEmpty(this.storeService.currentSection)) {
      ...
      this.subs.sink = this.form
        ...
        ?.valueChanges.subscribe((d) => {
          if (Object.keys(this.modelData).length !== 0) {
            this.bindModelObjectData();
          }
          if (d !== this.selectedValue) {
            this.bindingselectedsection(d);
          }
        });
    }
  }

  bindingselectedsection(d) {
    ...
    this.subs.sink = forkJoin([
      ...
      if (modelindex === -1) {
        ...
        this.modelData = JSON.parse(
          JSON.stringify(this.storeService.modelobjectDatat[modelindex]),
          reviver
        );
      }
    });
  ...
  getSectionDetails() {
    this.subs.sink = forkJoin([
      this.pageBuilderService.get('SectionDetail'),
      this.actionService.get(this.storeService.currentsectionId.id, 'Section'),
    ]).subscribe(([fields, model]) => {
      this.storeService.setLocaldomain(fields[0].LocalDomain);
      this.fields = [fields[3]];
      this.modelData = JSON.parse(JSON.stringify(model), reviver);
      this.storeService.currentSection = {};
    });
    this.storeService.currentSection = {};
  }
  ...
  bindModelObjectData() {
    if (this.field.key === 'SectionView') {
      ...
      this.storeService.currentSection = this.selectedValue;
      this.storeService.currentsectionId = isNotNilOrEmpty(
